...
impl ServiceDetector {
    ...
    async fn is_port_open(&self, ip: IpAddr, port: u16, protocol: &str) -> Result<bool> {
        match protocol {
            ...
            _ => Err(NmapError::UnsupportedProtocol),
        }
    }
    ...
    async fn grab_tcp_banner(&self, ip: IpAddr, port: u16) -> Result<String> {
        ...
        if !probe.is_empty() {
            stream.write_all(probe).await.map_err(|_| NmapError::SendFailed)?;
        }
        ...
        ).await.map_err(|_| NmapError::Timeout)?.map_err(|_| NmapError::ReceiveFailed)?;

        if n > 0 {
            buffer.truncate(n);
            Ok(String::from_utf8_lossy(&buffer).to_string())
        } else {
            Err(NmapError::NoBanner)
        }
    }
    ...
    async fn grab_udp_banner(&self, _ip: IpAddr, _port: u16) -> Result<String> {
        ...
        Err(NmapError::UnsupportedProtocol)
    }
    ...
    async fn send_tcp_probe(&self, ip: IpAddr, port: u16, probe: &Probe) -> Result<String> {
        ...
        // Send probe data
        stream.write_all(&probe.data).await.map_err(|_| NmapError::SendFailed)?;
