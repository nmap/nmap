...
Engine =
{
  ...
  new = function(self, driver, host, port, options)
    ...
    setmetatable(o, self)
    self.__index = self
    o.max_threads = stdnse.get_script_args("brute.threads") or 100
    return o
  end,
...
-- mark:doAuthenticate
  ...
  doAuthenticate = function( self )
    ...
    repeat
      ...
      status, response, report = driver:connect()

      -- if the result of login is unsuccessful and the script is adaptible
      -- for example connection was RSTed
      if ( not(status) and report ) then
        response = Error:new("Report was provided, stopped ...")
        return false, response, report
      end
      ...
      if ( status ) then
        if ( not(username) and not(password) ) then
          repeat
            if ( #self.retry_accounts > 0) then
              ...
              username = self.retry_accounts[#self.retry_accounts][1]
              password = self.retry_accounts[#self.retry_accounts][2]
        ...
        status, response, report = driver:login( username, password )
...
-- mark:login
  login = function(self, cvar )
    ...
    while( true ) do
      ...
      if not( self.batch_full ) and self.batch_assembling and not ( thread_data.in_batch ) then
      ...
      local status, response, report = self:doAuthenticate()
      ...
      elseif ( report ) then
        ...
        self.retry_accounts[#self.retry_accounts + 1] = { report.creds.username, report.creds.password }
        thread_data.error = true
        while thread_data.error do
          condvar "signal"
          condvar "wait"
        end
      else
  ...
  addNewWorker = function(self, cvar)
    ...
    self.threads[co].error = nil
    self.threads[co].attempt = 0
    self.threads[co].in_batch = false
    self.threads[co].ready = false
    return co
  end,
  ...
  renewBatch = function(self)
    ...
    self.batch_start_time = self.tick
    self.batch_full = false
    self.batch_assembling = true
  ...
  start = function(self)
    ...
    while self:threadCount() > 0 do
      ...
      for co, v in pairs( self.threads ) do
        if v.error then
          -- is it a new error, or old one?
          if v.attempt >= batch_start_time then
            error_since_batch_start = true
          end

          if not ( killed_one ) then
            stdnse.debug1("NOTICE: killed one")
            v.terminate = true
            killed_one = true

            -- we want it dead as soon as possible, so we want
            -- to be sure that the thread is going to run.
        ...
        -- if there were an error since we started to assemble a batch
        -- discard the current batch and start a new one
        if error_since_batch_start and self.batch_assembling then
          self:renewBatch()
        end
      end


      if not( killed_one ) and self.batch_assembling and self.batch_full and num_threads < self.max_threads then
        self:addNewWorker(cvar)
      ...
      stdnse.debug1("BASE thread #threads = %d", num_threads)
