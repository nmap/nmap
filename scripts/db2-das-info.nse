...
function parse_db2_packet(packet)
  ...
  local _, len = string.unpack(">I2", packet.data:sub(info_length_offset, info_length_offset + 1))
  _, response.version = string.unpack("z", packet.data:sub(version_offset) )
...
function read_db2_packet(socket)
  ...
  if packet.header.raw:sub(1, 10) == "\x00\x00\x00\x00\x44\x42\x32\x44\x41\x53" then
    ...
    local _, endian = string.unpack( "c2" .. packet.header.raw .. ENDIANESS_OFFSET )

    if endian == "9z" then
      _, packet.header.data_len = string.unpack("<I4", packet.header.raw, DATA_LENGTH_OFFSET )
    else
      _, packet.header.data_len = string.unpack(">I4", packet.header.raw, DATA_LENGTH_OFFSET )
    end
