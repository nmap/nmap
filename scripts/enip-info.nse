...
--|   deviceType: Communications Adapter (12)
--|   productCode: 158
--|   revision: 3.7
--|_  ipAddress: 192.168.1.1
-- @xmloutput
...
--<elem key="deviceType">Communications Adapter (12)</elem>
...
action = function(host,port)
	...
	if (command == 0x63) then
		...
		if(typeId == 0x0c) then
			-- device type number
			local devnum
			devnum = string.unpack("<I2", response, 51)
			-- lookup device type based off number, return to output table
			output["type"] = device_type_lookup(devnum) .. " (" .. devnum .. ")"
			-- vendor number
			local vennum
			vennum = string.unpack("<I2", response, 49)
			...
			output["productName"] = string.unpack("s1", response, 63)
			-- unpack the serial number
			local serial
			serial = string.unpack("<I4", response, 59)
			-- print it out in hex format
			output["serialNumber"] = string.format("%#0.8x", serial)
			-- unpack product code as a two byte int
			output["productCode"] = string.unpack("<I2", response, 53)
			-- Revision Nuumber
			local char1, char2
			char1, char2 = string.unpack("BB", response, 55)
			output["revision"] = char1 .. "." .. char2
			-- Device IP, this could be the same, as the IP scanning, or may be actual IP behind NAT
			local dword
			dword = string.unpack(">I4", response, 37)
			output["deviceIp"] = ipOps.fromdword(dword)
			-- set Nmap output
			set_nmap(host, port)
			-- close socket
			socket:close()
