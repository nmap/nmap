...
description = [[
Initiates a TLS 1.2 handshake with the target and enumerates the supported cipher suites and compression methods. For each cipher suite, the script assigns a letter grade (A through F) based on its cryptographic strength, providing a clear assessment of the server’s TLS 1.2 security posture.
]]

---
-- @usage
-- nmap -sV --script tls1.2-enum-cipher -p 443 <host>
--
-- @output
-- PORT    STATE SERVICE
-- 443/tcp open  https
...
author = {"Braiant Giraldo <iron.fortress@gmail.com>", "Braiant Giraldo"}
      ...
      if server_hello.protocol ~= protocol then
        ...
        if not remove(group, name) then
          ...
          if scores then
            ...
            local ske
            if kex.server_key_exchange then
...
action = function(host, port)
  ...
  -- Only test TLSv1.2
  stdnse.debug1("Trying protocol TLSv1.2.")
  local co = stdnse.new_thread(try_protocol, host, port, "TLSv1.2", results)
  threads[co] = true
