    ...
    if status then
      ...
      elseif ospf_raw:byte(1) == 0x02 and ospf_raw:byte(2) == OSPF_MSG_LSUPD then
        ...
        if next(targets) then
          local out_links   = {}
          out_links['name'] = "External Routes"
          table.insert(output, "Area ID: " .. ipOps.fromdword(ospf_ls_upd.header.area_id))
...
action = function()
  ...
  local output = ospfListen(interface, timeout)
  if output then
