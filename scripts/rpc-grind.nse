...
local isRPC = function(host, port)
  ...
  if port.service == 'rpcbind' then
    ...
    if not status then
      ...
      _, rxid = string.unpack(">I4", data, 1)
      _, msgtype = string.unpack(">I4", data, 5)
  ...
  for program, number in iterator do
    ...
    if type(response) == 'table' then
      ...
      if response.accept_state == rpc.Portmap.AcceptState.PROG_MISMATCH then
        ...
        _, result.highver = string.unpack(">I4", data, #data - 3)
        _, result.lowver = string.unpack(">I4", data, #data - 7)
