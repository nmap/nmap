...
Driver =
{
  ...
  login = function( self, username, password )
   local status, data, try
    data = string.pack("b", 0x6) .. "/login" .. string.pack("x")
    ...
    if ret then
        ...
        local md5str = string.pack("x") .. password .. stdnse.fromhex(ret) --appends pwd and challenge
        local chksum = stdnse.tohex(openssl.md5(md5str))
        local user_l = username:len()+6 --we add six because of the string "=name="
        local login-pkt = string.pack("b", 0x6) .. "/login" .. string.pack("b", user_l) .. "=name=" .. username .. string.pack("b", 0x2c) .. "=response=00" .. chksum .. string.pack("x")
