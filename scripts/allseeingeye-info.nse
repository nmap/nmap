...
action = function(host, port)
  ...
  local pos = 5
   ...
   o["game"], pos = string.unpack("s1", data, pos)
   o["port"], pos = string.unpack("s1", data, pos)
   o["server name"], pos = string.unpack("s1", data, pos)
   o["game type"], pos = string.unpack("s1", data, pos)
   o["map"], pos = string.unpack("s1", data, pos)
   o["version"], pos = string.unpack("s1", data, pos)
   o["passworded"], pos = string.unpack("s1", data, pos)
   o["num players"], pos = string.unpack("s1", data, pos)
   o["max players"], pos = string.unpack("s1", data, pos)
  ...
  while data:byte(pos) ~= 1 do
    local key, value
     key, value, pos = string.unpack("s1 s1", data, pos)
    kv[key] = value
  end
  pos = pos + 1
