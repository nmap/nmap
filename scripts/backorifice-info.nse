...
local function BOunpack(packet)
  local magic, pos = string.unpack(tostring(packet))

  if magic ~= MAGICSTRING then return nil,TYPE.ERROR end  --received non-BO packet

  local packetsize, packetid, type_packet, data
  packetsize, packetid, type_packet, pos = string.unpack("<I4I4B",packet,pos)
  data, pos = string.unpack("c"..(packetsize-pos-1),packet,pos)
