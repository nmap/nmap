...
local function enum_protocols(host, port)
  ...
  for k, v in pairs(PROTOCOLS) do
    ...
    local pos, success = string.unpack("B", response.itut.data)
    if ( success == 2 ) then
      table.insert(res_proto, ("%s: SUCCESS"):format(k))
    elseif ( nmap.debugging() > 0 ) then
      local pos, err = string.unpack("B", response.itut.data, 5)
      if ( err > 0 ) then
...
local function enum_ciphers(host, port)
  ...
  for k, v in get_ordered_ciphers() do
    ...
    comm:close()
    if ( status ) then
      local pos, enc_level = string.unpack("B", response.itut.data, 95 + 8)
      local pos, enc_cipher= string.unpack("B", response.itut.data, 95 + 4)
