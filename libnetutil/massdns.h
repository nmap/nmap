...
#ifndef NETUTIL_MASSDNS_H
#define NETUTIL_MASSDNS_H
...
struct Stats
{
  Stats() : servers(0), names(0), actual(0), ok(0), nx(0), sf(0), trans(0), dropped(0), system(0) {}

  int servers; // Number of servers used
  int names;   // Requests we attempt to resolve
  int actual;  // actual DNS queries we intend to make
  int ok;      // queries answered successfully
  int nx;      // NXDOMAIN (or failed system resolution)
  int sf;      // SERVFAIL
  int trans;   // queries transmitted
  int dropped; // queries dropped after failing with all servers
  int system;  // Requests deferred to system name resolution
...
public:
  ...
  void Init(Request *requests, int num_requests);
  void Resolve(bool system=false);
  ...
  bool isSystemDnsOK(const char **err) const;
  std::list<std::string> getServers() const;
