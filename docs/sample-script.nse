description = [[
This is an example script for the Nmap Scripting Engine. It is intended to be
a quick way to get started on developing a script without having to find and
...
your script does. The first paragraph will show up on the summary page, so ensure
it's a good high-level overview of your script. Following paragraphs will appear
when the user clicks on the script, and should outline what it does, how it works,
...
---Uncomment any of the following functions (as well as the 'require' line above) to use a simple portrule
-- See the documentation for the shortport library for more information:
...
---Finally, the action function. This is called once for each time the rule function returns true. 
-- The host and/or port may be nil depending on what type of rule fired. If you need more than one
-- rule type (for example, a prerule then a hostrule), scroll past this function. 
action = function( host, port )
  ...
  -- To display debug output, use stdnse.print_debug(). All output should be prefixed with the name
  -- of your script
  ...
  -- If your response is an error, you should return it with the stdnse.format_output() function
  -- (the string 'ERROR: ' will be appended, and it will only be displayed if debugging is enabled):
  -- return stdnse.format_output(false, "PC Load Letter")

  -- To create a socket, use the nmap.new_socket() function. See the online documentation here for
  -- more information on how to create and use sockets:
  -- http://nmap.org/nsedoc/lib/nmap.html#new_socket
  local s1 = nmap.new_socket('tcp')
  local s2 = nmap.new_socket('udp')

  -- Most socket functions, and many other library functions, return two values: a status and
  -- a result. The status should always be checked:
  local status, err = s1:connect(host or 'localhost', port or '80')
  if(not(status)) then
    return stdnse.format_output(false, "Couldn't connect socket: " .. err)
  end
  
  -- To inspect any variable, use the nsedebug.tostr() function. This should only be used for 
  -- debugging, not for actual output. 
  io.write(nsedebug.tostr(host))

  -- To display a string as hex, use the nsedebug.print_hex() function. This prints hex and,
  -- when possible, ascii. 
  nsedebug.print_hex(host.bin_ip)

  -- If you want your script to add more targets to the Nmap scan, you can use the target
  -- module. First require 'target' above, then perform the following:
  if(target.ALLOW_NEW_TARGETS) then
    target.add('192.168.1.1')
  end

  -- If your response is more complicated, you can build a table, potentially with subtables, and
  -- pass it to stdnse.format_output(). Each table can have a list of output values, numerically,
  -- which will be displayed in order. Additionally, they can have the 'name' key, which will be
  -- displayed at the top, and the 'warning' key, which will only be displayed if debugging is
  -- enabled. For more information and examples, see the documentation for stdnse.format_output().
...
---Uncomment the rest of the file and remove the action function above to use a dispatch table
-- instead of a single action. This lets you define different actions for prerule, hostrule, etc)
--
