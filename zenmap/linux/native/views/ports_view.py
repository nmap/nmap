...
class PortsView(Gtk.Box):
    def __init__(self, on_show_host_details) -> None:
        ...
        self._empty = empty_state("Run a scan to populate port results.")
        self._stack = Gtk.Stack()
        self._stack.set_vexpand(True)
        self._stack.add_named(self._table, "table")
        self._stack.add_named(self._empty, "empty")

        self._count_label = Gtk.Label(label="0 ports", xalign=1)
        header = section_header("Ports")
        header.append(self._count_label)
        self.append(header)
        self.append(filter_bar("Filter port results", self._set_filter))
        ...
        self._copy_button = Gtk.Button(label="Copy Host:Port")
        self._copy_button.connect("clicked", lambda *_: self._copy_host_port())
    ...
    def _refresh(self) -> None:
        ...
        count_text = (
            f"{len(filtered)} of {len(ports)} ports"
            if self._filter_text.strip()
            else f"{len(ports)} port result(s)"
        )
        self._count_label.set_label(count_text)
        if not ports:
            self._stack.set_visible_child_name("empty")
            self._empty.get_first_child().set_label("Run a scan to populate port results.")
        elif not filtered:
            self._stack.set_visible_child_name("empty")
            self._empty.get_first_child().set_label("No ports match the current filter.")
        else:
    ...
    def _copy_host_port(self) -> None:
        port = self.selected_port()
        if port is None:
            return
        clipboard = Gdk.Display.get_default().get_clipboard()
        clipboard.set(f"{port.host_address}:{port.port_number}")
