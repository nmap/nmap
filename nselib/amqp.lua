...
AMQP = {
  ...
  decodeTable = function(self, tbl, tsize)
    ...
    while read < tsize do
      ...
      tmp = select( 2, string.unpack("B", tmp) )
      ...
      if ( tmp == 'F' ) then -- table type
        ...
        read = read + 4
        value = {}
        tmp = select( 2, string.unpack(">I4", tmp) )
  ...
  decodeBoolean = function(self, key, read)
    ...
    value = select( 2, string.unpack("B", value) )
    read = read + 1
  ...
  handshake = function(self)
    ...
    if ( #tmp ~= 11 ) then
      if ( #tmp == 8 and select( 2, string.unpack(">I4", tmp) ) == 0x414D5150 ) then
        ...
        -- version unsupported
        v1, v2, v3, v4, _ = string.unpack(">BBBB", tmp, 5)
    ...
    frametype, chnumber, framesize, method, _ = string.unpack(">BI2I4I4", tmp)
    ...
    version = select( 2, string.unpack(">I2", tmp) )
    ...
    local tablesize = select( 2, string.unpack(">I4", tmp) )
    properties = {}
