    ...
    void *arg)
{
    ...
    ret = SSL_select_next_proto((unsigned char**)out, outlen, alpn, alpn_len, in, inlen);
    if (ret == OPENSSL_NPN_NEGOTIATED) {
        char *proto_str;

        // use o.sslalpn so we don't have to worry about freeing it later
        proto_str = safe_malloc(*outlen + 1);
        memcpy(proto_str, *out, *outlen);
        proto_str[*outlen] = 0;
        if (o.debug)
            logdebug("negotiated ALPN is %s\n", proto_str);

        *out = (const unsigned char*)strstr(o.sslalpn, proto_str);
        free(proto_str);
    }
    else {
        if (o.debug)
            logdebug("no ALPN Negotiated\n");
    }
    free(alpn);
...
#ifdef HAVE_ALPN_SUPPORT
    if (o.sslalpn) {
        SSL_CTX_set_alpn_select_cb(sslctx, alpn_select_cb, NULL);
    }
#endif
